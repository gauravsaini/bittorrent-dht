...
class DHT extends EventEmitter {
  constructor(opts = {}) {
    ...
    this._rpc =
      opts.krpc || krpc(Object.assign({ idLength: this._hashLength }, opts));
    ...
    this._bucketOutdatedTimeSpan =
      opts.timeBucketOutdated || BUCKET_OUTDATED_TIMESPAN;
    ...
    function subscribe(topic, nodeId) {
  ...
  announce(infoHash, port, cb) {
    ...
    if (this._host) {
      ...
      this._addPeer({ host: this._host, port: port || dhtPort }, infoHash, {
        host: this._host,
        port: dhtPort,
      });
    }
  ...
  _onfindnode(query, peer) {
    const target = query.a.target;
    if (!target)
      return this._rpc.error(peer, query, [
        203,
        "`find_node` missing required `a.target` field",
      ]);
  ...
  _ongetpeers(query, peer) {
    ...
    if (!infoHash)
      return this._rpc.error(peer, query, [
        203,
        "`get_peers` missing required `a.info_hash` field",
      ]);
  ...
  _onget(query, peer) {
    ...
    if (!value) {
      ...
      this._rpc.response(
        peer,
        query,
        createGetResponse(this._rpc.id, token, value)
      );
    }
  }
